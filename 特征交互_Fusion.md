...
为了解决上述问题，我们提出了一种多模态元学习（表示为 MML）方法，它将项目的多模态辅助信息（例如文本和图像）合并到元学习过程中，以稳定和改进冷学习的元学习过程。开始顺序推荐。具体来说，我们设计了一组与每种模态相对应的多模态元学习器，其中ID特征用于开发主要元学习器，其余文本和图像特征用于开发辅助元学习器。我们不是简单地组合来自不同元学习器的预测，而是设计一个自适应的、可学习的融合层来集成基于不同模态的预测。同时，我们设计了一个冷启动项目嵌入生成器，它利用多模态辅助信息来预热新项目的 ID 嵌入。大量的离线和在线实验表明，与基线模型相比，MML 可以显着提高冷启动用户的推荐性能。  
